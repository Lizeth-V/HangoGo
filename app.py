        ...
        for place_id in favorite_list:
            ...
            if place_info:
                favorites_info.append({
                    ...
                    "place_id": str(place_info.get("_id", "")),
                })
...
@app.route('/set_user_active_place', methods=['GET'])
def set_user_active():
    user = session.get('user')
    user_id = str(user.# app.py
from bson import ObjectId
import json
...
def set_user_active():
    ...
    temp_feedback.set_active_place(user_id=user_id, place_id=place_id)
