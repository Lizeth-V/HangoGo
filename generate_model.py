...
# Nhu's alternate version to Aidan's generate_place_probabilities function for multi_rec
def generate_multi_place(uids):
    import tensorflow as tf
    start = time.time()
    ...
    # query that allows for multiple user_ids
    user_json_list = []
    for user_id in uids:
        user_json_list.append({"user_id": user_id})

    combined_query = {
    "$or": user_json_list
    }

    review_table = list(collection.find(combined_query))
    ...
    return (list(unreviewed_place_ids))
